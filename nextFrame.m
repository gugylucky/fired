function [show, flag, finalBbox] = nextFrame( datavideo, thFrame, threshold, interval, minimumPixel, parameterLBPTOP, Offset )
...
% - datavideo berisi variabel volumedata_RGB dan gray, merupakan data keseluruhan frame pada video.
...
%% read frame
m1 = uint8(datavideo.volumedata_RGB(:,:,:,thFrame-interval-interval));
m2 = uint8(datavideo.volumedata_RGB(:,:,:,thFrame-interval));
m3 = uint8(datavideo.volumedata_RGB(:,:,:,thFrame));
...
area        = regionprops(lokasi_filtered,'Area');
    ...
    for k = 1 : length(bbox)
        ...
        if thisArea >= minimumPixel
            ...
            volData     = datavideo.volumedata_gray(thisBbox(2):thisBbox(2)+thisBbox(4),thisBbox(1):thisBbox(1)+thisBbox(3),thFrame-T-T:thFrame);
